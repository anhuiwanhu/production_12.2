...
%>
<body  class="docBodyStyle"  style="position:relative; height:100%;" >
<s:hidden name="govFieldJson" id="govFieldJson"  />
<input id="receiveFileFileNumberOld" type="hidden" value="<%=request.getAttribute("receiveFileFileNumberTemp")%>" />
             ...
             <td height="100%">
	            <div class="docbox_noline"   >
					   <div class="doc_Movetitle">
						 <ul>
							  <li class="aon"  id="Panle0"><a href="javascript:void(0);" onClick="changePanle(0);" ><%=Resource.getValue(local,"Gov","gov.psform")%></a>
                              </li>
...
$(document).ready(function() {
 ...
 /*   $("#receiveFileSendFileUnitTemp").combobox({
    ...
    $("#receiveFileSendFileUnitTemp").combobox({
        filter:function(q,row){
            ...
            for(i in diseaseList){
                if(diseaseList[i] == row.text){
                   //$("#diseaseName").combobox("setValues",diseaseList);
                    return;
                }
            }
            diseaseList.push(row.text);
            //$("#diseaseName").combobox("setValues",diseaseList);
            ...
            $("#receiveFileSendFileUnitTemp").combobox('clear');
        }
    ...
    function receiveFileReceiveDateInitVal(){
        if($("input[name='receiveFileReceiveDate']").length>0){
         var receiveFileReceiveDateVal=$("input[name='receiveFileReceiveDate']").val();
            if(receiveFileReceiveDateVal==""){
               $("input[name='receiveFileReceiveDate']").val('<%=nowdate%>');
            }
        }
    }
