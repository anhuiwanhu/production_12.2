...
if( "blcyedit".equals(  request.getParameter("from") ) ){
	...
	if(transactStatus.equals("1")){
		if("1".equals(processType)){
			//新流程 办理完毕 不支持退回功能
			modiButton=",Saveclose,Viewtext,EmailSend,Print,Readtext";
		}else{
...
if( "blcyview".equals(  request.getParameter("from") ) ){
	...
	modiButton=",Viewtext,Downtext,EmailSend,Print,";
}
...
<input type="hidden" name="isWordWindowFirst" value="1">
	...
	<input type="hidden" name="documentWordType" value="<%=request.getAttribute("documentWordType")%>">
	            ...
	            <div class="docbox_noline"  align="center" >
					   <div class="doc_Movetitle">
						 <ul>
							  ...
							  <li id="Panle2" ><a href="javascript:void(0);" onClick="changePanle(2);"><%=Resource.getValue(local,"Gov","gov.glflow")%><span class="redBold" id="viewrelationnum"></span></a></li>
							  <%if( !"1".equals( request.getAttribute("p_wf_pool_processType") ) ){%>
							  <li id="Panle3" ><a href="#" onClick="changePanle(3);"><%=Resource.getValue(local,"Gov","gov.xgfj")%><span class="redBold" id="viewaccnum"></span></a></li>
							  <%}%>
							  <li id="Panle4" ><a href="#" onClick="changePanle(4);"><%=Resource.getValue(local,"Gov","gov.flowgzd")%></a></li>
						 </ul>
					   </div>  
...
function  changePanle(flag){
	...
	if(flag=="4"){
		...
		<%}else{%>
		showWorkFlowTrackingList("docinfo4");
		<%}%>
	}
}
