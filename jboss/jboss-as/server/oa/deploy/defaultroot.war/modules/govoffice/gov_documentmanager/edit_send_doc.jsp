...
<%
    ...
    String dlcode = util1.getSysEncoderKeyVlaue("FileName",
            request.getAttribute("content")+"_1" +request.getAttribute("documentWordType"), "dir");

    File fileTemp = new File(request.getRealPath("") +"/upload/govdocumentmanager/"+ request.getAttribute("content")+"_1" +request.getAttribute("documentWordType"));
    Boolean fileFlag=fileTemp.exists();
    String  fileExist="0";
    if(fileFlag){
        fileExist="1";
    }
...
<head>
	<title><%=Resource.getValue(local,"Gov","gov.sendfile")%></title>
...
%>
<body  class="docBodyStyle"  style="position:relative; height:100%;"      onload="initBody();commentWidth();">
<input name="fileFlag" type="hidden" value="<%=fileExist%>" />
...
<input type="hidden" name="$accessoryName" value="-1">
...
<input type="hidden" name="sendToNameNew" id="sendToNameNew" value="<%=request.getAttribute("sendToName")==null?"":request.getAttribute("sendToName").toString()%>"/>
 ...
 <input type="hidden" name="fileVerifyCodeTemp" value="<%=dlcode%>" />
             ...
             <td height="100%">
	            <div class="docbox_noline"   >
					   <div class="doc_Movetitle">
						 <ul>
							  <li class="aon"  id="Panle0"><a href="javascript:void(0);" onClick="changePanle(0);" ><%=Resource.getValue(local,"Gov","gov.psform")%></a></li>
							  <li id="Panle1"><a href="javascript:void(0);" onClick="changePanle(1);"><%=Resource.getValue(local,"Gov","gov.flowpic")%></a></li>
							  <li id="Panle2" ><a href="#" onClick="changePanle(2);"><%=Resource.getValue(local,"Gov","gov.flowgzd")%></a></li>
							  <li id="Panle3" ><a href="#" onClick="changePanle(3);"><%=Resource.getValue(local,"Gov","gov.glflow")%><span class="redBold" id="viewrelationnum"></span></a></li>
							  <%if(! "1".equals( request.getAttribute("p_wf_pool_processType") ) ){%>
							  <li id="Panle4" ><a href="#" onClick="changePanle(4);"><%=Resource.getValue(local,"Gov","gov.xgfj")%><span class="redBold" id="viewaccnum"></span></a></li>
							  <%}%>
							  <li id="Panle5" ><a href="#" onClick="changePanle(5);"><%=Resource.getValue(local,"Gov","gov.xgjl")%></a></li>
							  <li id="Panle6" ><a href="#" onClick="changePanle(6);"><%=Resource.getValue(local,"Gov","gov.xgreceive")%></a><span class="redBold" id="viewReceivenum"></span></li>
                              <%if(oa_PDF.equals("1")){%>
                              <li id="Panle7" ><a href="#" onClick="changePanle(7);"><%=Resource.getValue(local,"Gov","gov.pdfpz")%></a><span class="redBold" id="viewpdfnum"></span></li>
						      <%}%>
                         </ul>
					   </div>  
                       ...
                       <div id="docinfo0" class="doc_Content"  align="center">
							...
							<div  align="center" > 
								...
								if (!file.exists()) {
                                    ...
                                    com.whir.govezoffice.documentmanager.common.util.GovFormUtil.getForm(request, tableId_form ,"0","0", file.lastModified());
                                }
 ...
 %>
     ...
     class="listTable">
        ...
        <TR class="listTableHead">
          <TD width="60%" class="td_lefttitle"><%=Resource.getValue(local,"Gov","gov.xiugaineirong")%></TD>
          <TD width="15%"><%=Resource.getValue(local,"Gov","gov.xiugairen")%></TD>
          <TD width="25%"><%=Resource.getValue(local,"Gov","gov.xiugaishijian")%></TD></TR>
 
        <TR cellpadding="0" class="listTableLine2">
          <TD colSpan=3 class="td_lefttitle"><B><%=Resource.getValue(local,"Gov","gov.govtitle")%></B></TD></TR>
        ...
        <TR cellpadding="0" class="listTableLine2">
          <TD colSpan=3 class="td_lefttitle"><B><%=Resource.getValue(local,"Gov","gov.zhusong")%></B></TD></TR>
        ...
        <TR cellpadding="0" class="listTableLine2">
          <TD colSpan=3 class="td_lefttitle"><B><%=Resource.getValue(local,"Gov","gov.chaosong")%></B></TD></TR>
		  ...
		  %>		  
		    <TR cellpadding="0" class="listTableLine2">
          <TD colSpan=3 class="td_lefttitle"><B><%=Resource.getValue(local,"Gov","gov.neibufasong")%></B></TD></TR>
...
//if( flag == 3 ) flag= 2;
	...
	if(flag=="2"){
		...
		<%}else{%>
		showWorkFlowTrackingList("docinfo2");
		<%}%>
	}
	//显示关联流程
	if(flag=="3"){
...
function closeWin(){	
	$.dialog({id:'bufawindow111'}).close();
}
<s:if test="p_wf_cur_ModifyField.indexOf('$sendFileDepartWord$') != -1 || #parameters.isEdit != null">
$("*[name='sendFileDepartWord']").combobox({
    onSelect: function () {
        changeSenddocumentWord();
    }
});
</s:if>
...
function commentWidth(){
   var  table=$("#docinfo0").find("table").first();
