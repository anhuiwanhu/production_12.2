 ...
 if("1".equals(request.getAttribute("isFastDoc"))){
	...
	function cmdSendclose(){
		 if(document.getElementById("flag_savefile") != undefined  && $("*[name='contentAccSaveName']").val() == ""  ){
			  alert("<%=Resource.getValue(local,"Gov","gov.nhmysczswj")%>");
			  return false;
		  }
		  ...
		  if(document.getElementById("flag_ChangeNumber") != undefined  && document.getElementsByName("documentSendFileByteNumber")[0].value == ""  ){
			  alert("<%=Resource.getValue(local,"Gov","gov.govnhmybh")%>");
			  return false;
		  }
		//if($("input[name='p_wf_pool_processType']")[0].value == "1"){
			...
			$.dialog({id:'SendcloseWinId',title:'分发',width:620,height:400,content: 'url:'+hhref+''});
	}
					...
					<div class="docbox_noline"   >
						   <div class="doc_Movetitle">
							 <ul>
								  ...
								  <li id="Panle1" ><a href="javascript:void(0);" onClick="changePanle(1);"><%=Resource.getValue(local,"Gov","gov.glflow")%><span class="redBold" id="viewrelationnum"></span></a></li>
                                 <li id="Panle2"><a href="javascript:void(0);" onClick="changePanle(2);"><%=Resource.getValue(local,"Gov","gov.flowpic")%></a></li>
...
<script type="text/javascript">
 ...
 $(function(){
     var ngdwmc="<s:property value="ngdwmc" />";
     $("input[type='text'][name='documentSendFileWriteOrg']").val(ngdwmc);
     //初始化加载机关代字
     var lengthTemp=$("#sendFileDepartWordTest").length;
     if(lengthTemp>0){
         var optionLength=$("#sendFileDepartWordTest option").size();
         if(optionLength==1){
