					  	...
					  	<td>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0"  id="tableSource" bordercolordark="#E1E1E1"  class="SubTable">
								...
								<% if(null!= request.getAttribute("themeOptionList")&&!"NULL".equals(request.getAttribute("themeOptionList"))){%>
									...
									<%if("1".equals(show)){%>                                     
			                          <iframe name="accessoryIframe<%=i%>" id="accessoryIframe<%=i%>" src="<%=rootPath%>/public/upload/uploadify/upload_include.jsp?accessType=iframe&dir=quesCustAnswer&uniqueId=accessoryIframe<%=i%>&realFileNameInputId=opImgRealName<%=i%>&saveFileNameInputId=opImgSaveName<%=i%>&canModify=no&uploadLimit=1&buttonText=上传图片&fileTypeExts=*.jpg;*.jpeg;*.gif;*.png;&thumbnail=120x120_slt"  scrolling=""  border="1" frameborder="0" width="380" height="90" style="float:left"></iframe>
                                    <%}else{%>                                     
			                          <iframe name="accessoryIframe<%=i%>" id="accessoryIframe<%=i%>" src="<%=rootPath%>/public/upload/uploadify/upload_include.jsp?accessType=iframe&dir=quesCustAnswer&uniqueId=accessoryIframe<%=i%>&realFileNameInputId=opImgRealName<%=i%>&saveFileNameInputId=opImgSaveName<%=i%>&canModify=yes&uploadLimit=1&buttonText=上传图片&fileTypeExts=*.jpg;*.jpeg;*.gif;*.png;&thumbnail=120x120_slt"  scrolling=""  border="1" frameborder="0" width="380" height="90" style="float:left"></iframe>
                                    <%}%> 
                                    </td>
