...
function save(type) {
	...
	if(pat.length>0){
		...
		if(v1 == "someScope"){
			//alert("|"+$("input[name='participantGivenOrg']")[0].value+"|");
			if($("input[name='participantGivenOrg']")[0].value == ""){
	...
	if(taskNeedReadRadio.length > 0 && taskNeedReadRadio[0].value == "1") {
		...
		if(pat.length >0){
			...
			if(v1 == "someScope"){
				if(  $("input[name='passRoundGivenOrg']")[0].value == ""){
	...
	//whir:taskright 活动权限
	var taskright = $("select[name='taskright']").attr("value");
	if(taskright != null) {
		model.setAttribute("whir:taskright", taskright);
	}else{
		model.removeAttribute("whir:taskright");
	}
	...
	if(nodeCommentFieldSelect.length > 0 ){
		...
		if( nodeCommentField == 'nullCommentField' || nodeCommentField == 'autoCommentField' ){
			...
			if( nodeCommentFieldSelect.length > 0){
				...
				//批示意见 职位级别
				var nodeCommentFieldDutyLevelCheckObj =$("input[name='nodeCommentFieldDutyLevelCheck']:checked");
				if( nodeCommentFieldDutyLevelCheckObj.length > 0){
					model.setAttribute("whir:nodeCommentFieldDutyLevelCheck", nodeCommentFieldDutyLevelCheckObj[0].value);
					model.setAttribute("whir:nodeCommentFieldOperate", $("select[name='nodeCommentFieldOperate']")[0].value);
					model.setAttribute("whir:nodeCommentFieldDutyLevel", $("select[name='nodeCommentFieldDutyLevel']")[0].value);
					model.setAttribute("whir:nodeCommentFieldsOther", $("select[name='nodeCommentFieldsOther']")[0].value);
				}else{
					model.setAttribute("whir:nodeCommentFieldDutyLevelCheck", "");
					model.setAttribute("whir:nodeCommentFieldOperate", "");
					model.setAttribute("whir:nodeCommentFieldDutyLevel", "");
					model.setAttribute("whir:nodeCommentFieldsOther", "");
				}
			}else{
		...
		for( i = 0; i < btnCheckBox.length; i++) {
			if(btnCheckBox[i].checked) {
				...
				extension.set("id",name);//设置name属性
				{
					...
					if (name=="EzFlowCompulsoryEnd") extension.set("name","强制结束");
				...
				if(name == "EzFlowTransfer"){
					...
					var keepTranRecordCheckBox = $("input[name='keepTranRecord']:checked"); 
					if(keepTranRecordCheckBox.length > 0 ){
						extension.set("keepTranRecord", keepTranRecordCheckBox[0].value);
					}
				}
				model.addWhirExtension(extension);
			}
...
function initData(id) { 
	...
	*/
	...
	//whir:taskright	活动权限
	var taskright = model.getAttribute("whir:taskright");
	if(taskright != null) {
		$("select[name='taskright']").attr("value", taskright);
	}
	...
	for(i =0 ;i< extensionArray.length;i ++ ) {
		...
		//如果是转办按钮，则增加自动返回autoTranReturn，增加保留转办记录keepTranRecord
		if(name == "EzFlowTransfer"){
			...
			var keepTranRecordvalue = extension.get("keepTranRecord");
			if( keepTranRecordvalue != null ){
				var keepTranRecordCheckBox = $("input[name='keepTranRecord'][value='"+ keepTranRecordvalue+"']"); 
				if(keepTranRecordCheckBox.length > 0 ){
					keepTranRecordCheckBox[0].checked = false;
					keepTranRecordCheckBox[0].click();//click一下就变true,并触发事件
				}
			}
		}
	...
	if(nodeCommentFieldSelect.length > 0 && nodeCommentField != null){
		if (nodeCommentField == "nullCommentField" || nodeCommentField == "autoCommentField"  ){
			...
			//批示意见对应字段 职位级别
			var nodeCommentFieldDutyLevelCheck = model.getAttribute("whir:nodeCommentFieldDutyLevelCheck");
			var nodeCommentFieldDutyLevelCheckBox = $("input[name='nodeCommentFieldDutyLevelCheck']");
			//alert("nodeCommentFieldDutyLevelCheck:"+nodeCommentFieldDutyLevelCheck);
			if(nodeCommentFieldDutyLevelCheckBox.length > 0) {
				if(nodeCommentFieldDutyLevelCheck =='true' || nodeCommentFieldDutyLevelCheck ==true){
					nodeCommentFieldDutyLevelCheckBox[0].checked = true;
				}else{
					nodeCommentFieldDutyLevelCheckBox[0].checked = false;
				}
			}

			var nodeCommentFieldOperateSelect =model.getAttribute("whir:nodeCommentFieldOperate");
			var nodeCommentFieldDutyLevelSelect =model.getAttribute("whir:nodeCommentFieldDutyLevel");
			//alert("nodeCommentFieldOperateSelect:"+nodeCommentFieldOperateSelect);
			//alert("nodeCommentFieldDutyLevelSelect:"+nodeCommentFieldDutyLevelSelect);
			if(nodeCommentFieldOperateSelect !=null && nodeCommentFieldOperateSelect !=undefined && nodeCommentFieldOperateSelect !=''){
				$("select[name='nodeCommentFieldOperate']")[0].value = model.getAttribute("whir:nodeCommentFieldOperate");
			}
			if(nodeCommentFieldDutyLevelSelect !=null && nodeCommentFieldDutyLevelSelect !=undefined && nodeCommentFieldDutyLevelSelect !=''){
				$("select[name='nodeCommentFieldDutyLevel']")[0].value = model.getAttribute("whir:nodeCommentFieldDutyLevel");
			}
			var nodeCommentFieldsOtherSelect =model.getAttribute("whir:nodeCommentFieldsOther");
			//alert("nodeCommentFieldsOtherSelect:"+nodeCommentFieldsOtherSelect);
			if(nodeCommentFieldsOtherSelect !=null && nodeCommentFieldsOtherSelect !=undefined && nodeCommentFieldsOtherSelect !=''){
				$("select[name='nodeCommentFieldsOther']")[0].value = model.getAttribute("whir:nodeCommentFieldsOther");
			}
		}
	}
...
function showTranTR(){
	if(getOperButtonCheckByValue("EzFlowTransfer")){
		$("#tranTR").show();
		//$("input[name='keepTranRecord']").prop("checked",true);
	}else{
		$("#tranTR").hide();
		//$("input[name='keepTranRecord']").prop("checked",false);
	}
...
function changerFormKey(formKey){ 
	...
	//其他人批示意见
	var nodeCommentFieldsOtherSelect = $("select[name='nodeCommentFieldsOther']");
	//清空
	if(nodeCommentFieldsOtherSelect.length >0 ){
		var alloptions = nodeCommentFieldsOtherSelect[0].options;
		for( j=alloptions.length-1;j>=0;j--){
			nodeCommentFieldsOtherSelect[0].remove(j);
		}
	}
	if(nodeCommentFieldsOtherSelect.length >0){
		nodeCommentFieldsOtherSelect.append('<option value="autoCommentField" selected>自动增加批示意见</option>');
		nodeCommentFieldsOtherSelect.append('<option value="nullCommentField">无批示意见</option>');
	}
	...
	form2Select.append("<option value='condi_preTransactorDutyLevel'>"+workflowMessage_js.JobLevelofLastActivityReviewer+"</option>");
	form2Select.append("<option value='condi_initiatorLeaderDutyLevel'>"+workflowMessage_js.LeaderJobLevelOfStarter+"</option>");   
	form2Select.append("<option value='condi_preTransactorLeaderDutyLevel'>"+workflowMessage_js.LeaderJobLevelofLastActivityReviewer+"</option>");
	form2Select.append("<option value='condi_initiatorOrgName'>"+workflowMessage_js.OrganizationOfStartPeople+"</option>");
	form2Select.append("<option value='condi_initiatorLeaderUserAccount'>"+workflowMessage_js.initiatorLeaderUserAccount+"</option>");
	form2Select.append("<option value='condi_initiatorDepartLeaderUserAccount'>"+workflowMessage_js.initiatorDepartLeaderUserAccount+"</option>");
	form2Select.append("<option value='condi_initiatorChargeLeaderUserAccount'>"+workflowMessage_js.initiatorChargeLeaderUserAccount+"</option>");
	...
	//if(field1Select.length >0 ){
		$(result).find("field").each(function(i){    
			...
			if(fieldtype == '401'){
				...
				nodeCommentFieldsOtherSelect.append("<option value='"+fieldid+"'>"+fieldtext+"</option");
...
//选择活动权限 赋值到操作按钮
function changerTaskright(obj){
	$("input[name='operButton']").each(function(){
		//alert($(this).val()+"|"+$(this)[0].checked);
		if($(this)[0].checked){
			$(this)[0].click();
		}
	});
	//alert($(obj).val());
	var taskRightButtonScopeIds =$(obj).find("option:selected").attr("buttonscope");
	//alert(taskRightButtonScopeIds);
	//EzFlowBackTask|0,EzFlowAddSignTask|3,EzFlowTranRead|3,EdAddSign|3,
	//EzFlowTransfer|3|1,
	//EzFlowCompulsoryEnd,EzFlowAbandon,EzFlowSendReadTask,EzFlowRelationProcess,EzFlowReCall,CancelTran,EzFlowFeedback,
	//EzFlowDrwaBack,EzFlowCancel,EzFlowNewProcess,EzFlowPress,EzFlowTranWithMail,EzFlowPrint,SynToInfo
	if(taskRightButtonScopeIds !=null && taskRightButtonScopeIds !=undefined && taskRightButtonScopeIds !=''){
		var taskRightButtonScopeIdsObj =taskRightButtonScopeIds.split(',');
		for(var i=0;i<taskRightButtonScopeIdsObj.length;i++){
			var buttonScopeIds =taskRightButtonScopeIdsObj[i];
			var buttonScopeIdsObj =null;
			var name ='';
			var range = '';
			var autoTranReturn ='';
			//alert("buttonScopeIds:"+buttonScopeIds);
			//退回【待办按钮】 || 加签【待办按钮】 || 转阅【待阅按钮】 || 补签【在办按钮】|| 转办【待办按钮】 || 分发【待办按钮】
			if(buttonScopeIds.indexOf('EzFlowBackTask') > -1 || buttonScopeIds.indexOf('EzFlowAddSignTask') > -1 || buttonScopeIds.indexOf('EzFlowTranRead') > -1 ||
			   buttonScopeIds.indexOf('EdAddSign') > -1 || buttonScopeIds.indexOf('EzFlowTransfer') > -1 || buttonScopeIds.indexOf('Sendclose') > -1){
				buttonScopeIdsObj =buttonScopeIds.split('|');
				name =buttonScopeIdsObj[0];
				range =buttonScopeIdsObj[1];
				//alert("name:"+name+"|range:"+range);
				...
				var btnCheckBoxRange = $("input[name='"+name+"'][value='" + range + "']");
				if(btnCheckBoxRange.length > 0) {
					btnCheckBoxRange[0].checked = false;
					btnCheckBoxRange[0].click();//click一下就变true,并触发事件
				}

				if(name =='EzFlowTransfer'){//转办【待办按钮】
					autoTranReturn =buttonScopeIdsObj[2];
					//alert("autoTranReturn:"+autoTranReturn);
					var autoTranReturnCheckBox = $("input[name='autoTranReturn'][value='"+ autoTranReturn+"']"); 
					if(autoTranReturnCheckBox.length > 0 ){
						autoTranReturnCheckBox[0].checked = false;
						autoTranReturnCheckBox[0].click();//click一下就变true,并触发事件
					}

					if(buttonScopeIdsObj.length>3){
                            var keepTranRecordVal =buttonScopeIdsObj[3];
                            //alert("autoTranReturn:"+autoTranReturn);
                            var keepTranRecordBox = $("input[name='keepTranRecord'][value='"+ keepTranRecordVal+"']"); 
                            if(keepTranRecordBox.length > 0 ){
                                keepTranRecordBox[0].checked = false;
                                keepTranRecordBox[0].click();//click一下就变true,并触发事件
                            }
                    }
				}
			}else{
