...
function initData() {
   ...
   //不可激活到的节点
   var  nojihuoActIds = model.getAttribute("whir:nojihuoActIds");

   var  nowaitingBaohanGatys = model.getAttribute("whir:nowaitingBaohanGatys");
    
	//取所有的活动
	var nms = model.metaNodeModels;
	for( i = 0; i < nms.length; i++) { 
		//>0为子流程的活动 ， 

		var uId=nms[i].getID();
		var uText=nms[i].getText(); 
		if( uId.indexOf("usertask")>= 0 || uId.indexOf("autobacktask")>= 0   ){  
			var actInfos= ''; 
			if(nojihuoActIds!=null&&nojihuoActIds.indexOf("$"+uId+"$")>=0){
				actInfos='<input name="nojihuoActId" type="checkbox" value="'+uId+'" checked>'+uText+' &nbsp;';	  
			}else{
				actInfos='<input name="nojihuoActId" type="checkbox" value="'+uId+'">'+uText+' &nbsp;';	 
			} 

			$("#nojihuoallActIdsDiV").append(actInfos); 
		}

		if( uId.indexOf("inclusivegateway")>= 0 || uId.indexOf("inclusivegateway")>= 0   ){  
			var actInfos= ''; 
			if(nowaitingBaohanGatys!=null&&nowaitingBaohanGatys.indexOf("$"+uId+"$")>=0){
				actInfos='<input name="nowaitingBaohanGaty" type="checkbox" value="'+uId+'" checked>'+uText+' &nbsp;';	  
			}else{
				actInfos='<input name="nowaitingBaohanGaty" type="checkbox" value="'+uId+'">'+uText+' &nbsp;';	 
			} 

			$("#nowaitingBaohanGatyDiV").append(actInfos); 
		}
	}
	...
	//whir:processEndRtx		结束即时通讯提醒  true/false
	var processEndRtxRadio = $("input[name='processEndRtx'][value='" + model.getAttribute("whir:processEndRtx") + "']");
	if(processEndRtxRadio.length > 0) {
		processEndRtxRadio[0].checked = true;
	}

	//whir:processEndSms		结束短信提醒  true/false
	var processEndSmsRadio = $("input[name='processEndSms'][value='" + model.getAttribute("whir:processEndSms") + "']");
	if(processEndSmsRadio.length > 0) {
		processEndSmsRadio[0].checked = true;
	}

	//whir:processNoBackSubmitUser		不允许退回给流程启动人  true/false
	var processNoBackSubmitUserRadio = $("input[name='processNoBackSubmitUser'][value='" + model.getAttribute("whir:processNoBackSubmitUser") + "']");
	if(processNoBackSubmitUserRadio.length > 0) {
		processNoBackSubmitUserRadio[0].checked = true;
	}
	...
	//whir:subTableDataControlAdd 允许新增行
	var subTableDataControlAdd = model.getAttribute("whir:subTableDataControlAdd");
	if(subTableDataControlAdd =='true' || subTableDataControlAdd ==true || subTableDataControlAdd =='false' || subTableDataControlAdd ==false) {
		var subTableDataControlAddInput = $("input[name='subTableDataControlAdd']");
		if(subTableDataControlAddInput.length > 0) {
			if(subTableDataControlAdd =='true' || subTableDataControlAdd ==true){
				subTableDataControlAddInput[0].checked = true;
			}else{
				subTableDataControlAddInput[0].checked = false;
			}
		}
	}else{
		var subTableDataControlAddInput = $("input[name='subTableDataControlAdd']");
	...
	//whir:subTableDataControlAddNull 新增默认空行
	var subTableDataControlAddNull = model.getAttribute("whir:subTableDataControlAddNull");
	if(subTableDataControlAddNull =='true' || subTableDataControlAddNull ==true || subTableDataControlAddNull =='false' || subTableDataControlAddNull ==false) {
		var subTableDataControlAddNullInput = $("input[name='subTableDataControlAddNull']");
		if(subTableDataControlAddNullInput.length > 0) {
			if(subTableDataControlAddNull =='true' || subTableDataControlAddNull ==true){
				subTableDataControlAddNullInput[0].checked = true;
			}else{
				subTableDataControlAddNullInput[0].checked = false;
			}
		}
	}else{
		var subTableDataControlAddNullInput = $("input[name='subTableDataControlAddNull']");
	...
	//whir:subTableDataControlDel 允许删除已有行数据
	var subTableDataControlDel = model.getAttribute("whir:subTableDataControlDel");
	if(subTableDataControlDel =='true' || subTableDataControlDel ==true || subTableDataControlDel =='false' || subTableDataControlDel ==false) {
		var subTableDataControlDelInput = $("input[name='subTableDataControlDel']");
		if(subTableDataControlDelInput.length > 0) {
			if(subTableDataControlDel =='true' || subTableDataControlDel ==true){
				subTableDataControlDelInput[0].checked = true;
			}else{
				subTableDataControlDelInput[0].checked = false;
			}
		}
	}else{
		var subTableDataControlDelInput = $("input[name='subTableDataControlDel']");
...
function save(type){
	...
	//whir:processEndRtx		结束即时通讯提醒  true/false
	var processEndRtxRadio = $("input[name='processEndRtx']:checked");
	if(processEndRtxRadio.length > 0) {
		model.setAttribute("whir:processEndRtx", processEndRtxRadio[0].value)
	}else{
		model.removeAttribute("whir:processEndRtx");
	}

	//whir:processEndSms		结束短信提醒  true/false
	var processEndSmsRadio = $("input[name='processEndSms']:checked");
	if(processEndSmsRadio.length > 0) {
		model.setAttribute("whir:processEndSms", processEndSmsRadio[0].value)
	}else{
		model.removeAttribute("whir:processEndSms");
	}

	//whir:processNoBackSubmitUser		不允许退回给流程启动人  true/false
	var processNoBackSubmitUserRadio = $("input[name='processNoBackSubmitUser']:checked");
	if(processNoBackSubmitUserRadio.length > 0) {
		model.setAttribute("whir:processNoBackSubmitUser", processNoBackSubmitUserRadio[0].value)
	}else{
		model.removeAttribute("whir:processNoBackSubmitUser");
	}
	...
	//whir:subTableDataControlAdd 允许新增行
	var subTableDataControlAddRadio = $("input[name='subTableDataControlAdd']:checked");
	if(subTableDataControlAddRadio.length > 0) {
		model.setAttribute("whir:subTableDataControlAdd", subTableDataControlAddRadio[0].value)
	}else{
		//model.removeAttribute("whir:subTableDataControlAdd");
		model.setAttribute("whir:subTableDataControlAdd", "false");
	}

	//whir:subTableDataControlAddNull 新增默认空行
	var subTableDataControlAddNullRadio = $("input[name='subTableDataControlAddNull']:checked");
	if(subTableDataControlAddNullRadio.length > 0) {
		model.setAttribute("whir:subTableDataControlAddNull", subTableDataControlAddNullRadio[0].value)
	}else{
		//model.removeAttribute("whir:subTableDataControlAddNull");
		model.setAttribute("whir:subTableDataControlAddNull", "false");
	}

	//whir:subTableDataControlModi 允许修改已有行数据
	//var subTableDataControlModiRadio = $("input[name='subTableDataControlModi']:checked");
	//if(subTableDataControlModiRadio.length > 0) {
	//	model.setAttribute("whir:subTableDataControlModi", subTableDataControlModiRadio[0].value)
	//}else{
	//	model.removeAttribute("whir:subTableDataControlModi");
	//}

	//whir:subTableDataControlDel 允许删除已有行数据
	var subTableDataControlDelRadio = $("input[name='subTableDataControlDel']:checked");
	if(subTableDataControlDelRadio.length > 0) {
		model.setAttribute("whir:subTableDataControlDel", subTableDataControlDelRadio[0].value)
	}else{
		//model.removeAttribute("whir:subTableDataControlDel");
		model.setAttribute("whir:subTableDataControlDel", "false");
	}
	...
	// 不可激活到的节点
	var nojihuoActIdsObj = $("input[name='nojihuoActId']:checked");//全部字段下拉框
	var nojihuoActIds_val="";
	if( nojihuoActIdsObj.length > 0 ){
		for (i=0;i<nojihuoActIdsObj.length ;i++ ){ 
			nojihuoActIds_val=nojihuoActIds_val+ "$"+nojihuoActIdsObj[i].value+"$"; 
		} 
	}

	model.setAttribute("whir:nojihuoActIds",nojihuoActIds_val);



    // 包含无需等待节点
    var nowaitingBaohanGatyObj = $("input[name='nowaitingBaohanGaty']:checked");//全部字段下拉框
	var nowaitingBaohanGaty_val="";
	if( nowaitingBaohanGatyObj.length > 0 ){
		for (i=0;i<nowaitingBaohanGatyObj.length ;i++ ){ 
			nowaitingBaohanGaty_val=nowaitingBaohanGaty_val+ "$"+nowaitingBaohanGatyObj[i].value+"$"; 
		} 
	}

	model.setAttribute("whir:nowaitingBaohanGatys",nowaitingBaohanGaty_val);
