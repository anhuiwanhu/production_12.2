...
String moduleId=request.getParameter("moduleId")==null?"1":com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"moduleId"); 
...
formType =com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(formType);
String subType=request.getParameter("subType")==null?"0":com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"subType");
...
//获取外部数据源配置-----开始
List outDataSourceListDeal =(List)request.getAttribute("outDataSourceListDeal");
String outDataSourceFieldsDealStr ="";
String outDataSourceCodeDealStr ="";
if(outDataSourceListDeal !=null && outDataSourceListDeal.size() >0){
	for(int i=0;i<outDataSourceListDeal.size();i++){
		Object[] objDeal =(Object[])outDataSourceListDeal.get(i);
		 outDataSourceFieldsDealStr = objDeal[2]==null?"":objDeal[2].toString();
		 outDataSourceCodeDealStr = objDeal[0]==null?"":objDeal[0].toString();
	}
}

List outDataSourceListBack =(List)request.getAttribute("outDataSourceListBack");
String outDataSourceFieldsBackStr ="";
String outDataSourceCodeBackStr ="";
if(outDataSourceListBack !=null && outDataSourceListBack.size() >0){
	for(int i=0;i<outDataSourceListBack.size();i++){
		Object[] objBack =(Object[])outDataSourceListBack.get(i);
		 outDataSourceFieldsBackStr = objBack[2]==null?"":objBack[2].toString();
		 outDataSourceCodeBackStr = objBack[0]==null?"":objBack[0].toString();
	}
}

String outDataSourceCodeStr ="";
if("".equals(outDataSourceCodeDealStr)){
	outDataSourceCodeStr =outDataSourceCodeBackStr;
}else{
	outDataSourceCodeStr =outDataSourceCodeDealStr;
}
//获取外部数据源配置-----结束
%>
<%@ page import="com.whir.component.extds.GlobalObject"%>
<%@ page import="com.whir.component.extds.DataSource"%>
...
<body class="Pupwin" onload="initData('<%=com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"id")%>')">
	<div class="BodyMargin_10">  
		...
		<s:form name="dataForm" id="dataForm" action="ezflowprocess!save.action" method="post" theme="simple" >
			...
			<input type="hidden" name="taskId" id="taskId" value="<%=com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"id")%>"> 
			<input type="hidden" name="outDataSourceFieldsDealStr" id="outDataSourceFieldsDealStr" value="<%=outDataSourceFieldsDealStr%>">
			<input type="hidden" name="outDataSourceFieldsBackStr" id="outDataSourceFieldsBackStr" value="<%=outDataSourceFieldsBackStr%>"> 
					...
					<td colspan="2">
						<div class="Public_tag">
							<ul>
								...
								<li id="Panle4" onClick="changePanle(4);"><span class="tag_center">外部数据源配置</span><span class="tag_right"></span>
								</li>
							</ul>
						</div>
					</td>
				...
				<tr>
					<td valign="top">
						...
						<table width="100%" border="0" class="Table_bottomline" cellpadding="0" cellspacing="0">
							...
							<!--活动权限-->
							<tr>
								<td  class="td_lefttitle">活动权限</td>
								<td>
								   <select name="taskright" id="taskright" class="selectlist" style="width:95%;" onChange="changerTaskright(this);" >
										<option value="">--请选择--</option>
										<c:forEach items="${taskrightList}" var="taskright">
											<option value="${taskright[0]}" buttonscope="${taskright[2]}">${taskright[1]}</option>
										</c:forEach>
							       </select>
								</td>
								<td width="60px">&nbsp;</td>
									...
									<tr>
										...
										<td class="subTitleList" style="border-right:1px solid #d9d9d9; border-bottom:1px solid #d9d9d9; padding:4px;">
											<select name="customOperateStr" id="customOperateStr"  class="selectlist"  style="width:70px;height:29px">
												<option value=">" selected><s:text name="workflow.newactivitydefinenewoperation5"/></option>
												<option value=">="><s:text name="workflow.newactivitydefinenewoperation7"/></option>
												<option value="<"><s:text name="workflow.newactivitydefinenewoperation1"/></option>
												<option value="<="><s:text name="workflow.newactivitydefinenewoperation8"/></option>
												<option value="=="><s:text name="workflow.newactivitydefinenewoperation4"/></option>
											</select>
										</td>
										<td class="subTitleList" style="border-right:1px solid #d9d9d9; border-bottom:1px solid #d9d9d9; padding:4px;">
										...
										<td  class="subTitleList" >
											<select  name="customPressUserIds_select" id="customPressUserIds_select" class="selectlist"  style="width:120px;height:29px">
										        <option value="$-1$"><s:text name="workflow.No"/></option>
												<option value="$-2$"><s:text name="workflow.handleperson"/><!--当前办理人--></option>
												<option value="$-3$"><s:text name="workflow.handlepersonLeader"/><!--当前办理人上级领导--></option>
												<option value="$-4$"><s:text name="workflow.handlepersonChargeLeader"/><!--当前办理人分管领导--></option>  
												<option value="$-5$"><s:text name="workflow.handlepersonDepartLeader"/><!--当前办理人部门领导--></option>
						...
						<tr id="show_box05" style="display: none">
							<td   align="left" valign="top"   class="td_lefttitle"><s:text name="workflow.activityduedeal"/>：</td>
						...
						<tr>
								...
								<table width="100%" cellpadding="0" cellspacing="0"  border="0"  style="margin-left:-4px;">
									...
									<tr>
										<td>
											...
											<INPUT value="true" type="checkbox" name="participantGroupNeedAllSend"><s:text name="workflow.defaultallsend"/><!--默认全部发送-->
										</td>
									</tr>
							...
							<tr>
								...
								<td>
									...
									<!--职位级别-->
								    &nbsp;&nbsp;&nbsp;<input name="nodeCommentFieldDutyLevelCheck" type="checkbox" value="true"><s:text name="workflow.dutyLevel"/> 
									<select name="nodeCommentFieldOperate">
										<option selected value="<">&lt;</option>
										<option value="<=">&lt;=</option>
										<option value="<>">≠</option>
										<option value="=">=</option>
									...
									<select name="nodeCommentFieldDutyLevel">
										<c:forEach items="${dutyList}" var="org">
											<option value="${org[2]}">${org[1]}(${org[2]})</option>
										</c:forEach>
									</select>
								    填写意见，其他人员
									<select name="nodeCommentFieldsOther">
										<option value="autoCommentField" selected><s:text name="workflow.newactivitycomment2"/></option>
										<option value="nullCommentField"><s:text name="workflow.newactivitycomment3"/></option>
									</select>
							...
							<tr>
							    <td  class="td_lefttitle" for='<s:text name="workflow.opinionAttitudeSetting"/>'><s:text name="workflow.opinionAttitudeSetting"/>：</td>
								<td>
								    <input name="commentAttitudeTypeSet"  checked  type="radio" value="0"><s:text name="workflow.notshowAttitude"/>
							        <input value="1" type="radio" name="commentAttitudeTypeSet"><s:text name="workflow.showagreeanddisagree"/>
									<input value="2" type="radio" name="commentAttitudeTypeSet"><s:text name="workflow.showreadagreedisagree"/>
								</td>
								<td>&nbsp;</td>
							</tr> 
							<!--需要阅件-->
									   ...
									   <tr>
										...
										<tr>
											<td>
												...
												<s:text name="workflow.activityallgroup"/>
												<INPUT value="true" type="checkbox" name="passRoundGroupNeedAllSend"><s:text name="workflow.defaultallsend"/>
										    </td>
											<td>&nbsp;</td>
										</tr> 
						    ...
						    <tr> 
								...
								<td>
									...
									<INPUT value="true" type="checkbox" name="processDealUserWithRepeat"><!--办理人重复时自动跳过--><s:text name="workflow.dealUserWithRepeat"/><img width="14" height="16" border="0" src="images/helpuser.gif"  title='办理人重复时自动跳过的设置项支持以下几种参与者方式：从系统角色中指定、流程启动人、上一活动办理人的上级领导、上一活动所有参与者、活动参与者本人、活动参与者上级领导、从选定的群组中选择、从候选人员中指定、指定全部办理人。'/>
								 </td>
								 <td></td>		
							  </tr> 
						...
						<div id="docinfo2" style="display:none;"> 
						  <table width="100%" border="0" class="Table_bottomline" cellpadding="0" cellspacing="0"  style="margin-left:4px;margin-top:4px;">
							...
							<tr>
								 ...
								 <input type="checkbox" value="1" name="keepTranRecord"  >不保留转办记录<!-- 不保留转办记录 -->
							   </td>
							   <td> 
							...
							<tr id="tranTR" style="display:none">
								<td colspan="2">
								    <div style="padding-left:66px;">
									  ...
									  <INPUT TYPE="radio" NAME="EzFlowTransfer"   value="1" onClick="showTranTR();"><s:text name="workflow.activitytranorg"/><!-- 本组织 -->
									  <INPUT TYPE="radio" NAME="EzFlowTransfer"   value="5" onClick="showTranTR();"><s:text name="workflow.activitytranorgandsub"/><!-- 本组织及下属组织 -->
						     ...
						     <tr id="addSignTR"  style="display:none">
				                 <td colspan="2">
								   <div style="padding-left:66px;">
									 ...
									 <INPUT TYPE="radio" NAME="EzFlowAddSignTask"   value="1" onClick="showAddSignTR();"><s:text name="workflow.activitytranorg"/><!-- 本组织 -->
									 <INPUT TYPE="radio" NAME="EzFlowAddSignTask"   value="5" onClick="showAddSignTR();"><s:text name="workflow.activitytranorgandsub"/><!-- 本组织及下属组织 -->
							...
							<tr>
								<td>
								   <INPUT TYPE="checkbox" NAME="operButton"  value="EzFlowCompulsoryEnd"><IMG SRC="images/toolbar/qzjs.png" align="absmiddle" width="24" height="24">&nbsp;&nbsp;<bean:message bundle="workflow" key="workflow.compulsoryEnd"/><!--强制结束-->【<bean:message bundle="workflow" key="workflow.newactivitybuttonunderapp"/>】
								</td>
								<td>&nbsp;</td>
							 </tr>
							 ...
							 <tr id="tranReadTR"  style="display:none">
								<td colspan="2">
								  <div style="padding-left:66px;">
									  ...
									  <INPUT TYPE="radio" NAME="EzFlowTranRead"   value="1" onClick="showTranReadTR();"><s:text name="workflow.activitytranorg"/><!-- 本组织 -->
									  <INPUT TYPE="radio" NAME="EzFlowTranRead"   value="5" onClick="showTranReadTR();"><s:text name="workflow.activitytranorgandsub"/><!-- 本组织及下属组织 -->
							  ...
							  <tr>
							    ...
							    <td><INPUT TYPE="checkbox" NAME="operButton"  value="CancelTran"><IMG SRC="images/toolbar/cancelTran.png">&nbsp;&nbsp<!-- 转办撤办 --><bean:message bundle="workflow" key="workflow.CancelTran"/>【<bean:message bundle="workflow" key="workflow.newactivitybuttonunfinished"/>】</td>
						     </tr>
							 
							  <tr id="edaddSignTR"  style="display:none">
				                 <td colspan="2">
								   <div style="padding-left:66px;">
								     <s:text name="workflow.buqianRange"/><!-- 补签范围 -->：
									 <INPUT TYPE="radio" NAME="EdAddSign"  value="0" onClick="showEdAddSignTR();"><s:text name="workflow.activitytranall"/><!-- 全部 -->
									 <INPUT TYPE="radio" NAME="EdAddSign"   value="1" onClick="showEdAddSignTR();"><s:text name="workflow.activitytranorg"/><!-- 本组织 -->
									 <INPUT TYPE="radio" NAME="EdAddSign"   value="5" onClick="showEdAddSignTR();"><s:text name="workflow.activitytranorgandsub"/><!-- 本组织及下属组织 -->
					   ...
					   <div id="docinfo4" style="display:none;">
							...
							<table width="100%" border="0" class="Table_bottomline" cellpadding="0" cellspacing="0" id="fieldTable"  style="margin-left:4px;margin-top:4px;" >
								<tr>
									<td width="15%" align="left"  for='从外部数据源中选择'>从外部数据源中选择：</td>
									<td width="85%" align="left">
										 <select name="ds" class="selectlist" style="width:30%;">
											<option value="localDataSource">本系统</option>
											<%
												List dsList = GlobalObject.getServiceIds(DataSource.class);
												if(dsList.size()>0){
													for(int i=0;i<dsList.size();i++){	
														Object obj=(Object)dsList.get(i);
											%>
											<option value="<%=obj.toString()%>" <%if(outDataSourceCodeStr.equals(obj.toString())){%>selected<%}%>><%=obj.toString()%></option>
											<%}}%>
										</select>&nbsp;
										<input name="dsbutton" type="button" class="btnButton6font" onclick="checkDS();" value="测试数据源">
									</td>
								</tr>
								<!------------办理中执行------------>
								<tr>
									<td width="100%" colspan="2" style="font-weight:bold;"><s:text name="workflow.newactivityworkflowmethod"/>：</td>
                                </tr>
								<%
								if(outDataSourceListDeal !=null && outDataSourceListDeal.size() >0){
									for(int i=0;i<outDataSourceListDeal.size();i++){
										Object[] objDeal =(Object[])outDataSourceListDeal.get(i);
										String outDataSourceSqlDeal =objDeal[1]==null?"":objDeal[1].toString();
										String outDataSourceFieldsDeal =objDeal[2]==null?"":objDeal[2].toString();
										String[] outDataSourceFieldsDealObj =outDataSourceFieldsDeal.split(",");
								%>
								<tr>
									<td width="100%" colspan="2">关联条件：<span class="MustFillColor">注：在输入框中输入SQL语句（如：insert into demo_user (user_id,user_name) values (hibernate_sequence.nextval, ?)|update demo_user set user_address =? where user_name =?），可写多条SQL语句，用“|”分隔。</span></td>
								</tr>
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding=0 cellspacing=0 border=0 width=100%>
											<tr>
												<td style="width:99%">
													<textarea name="outDataSourceSql" class="inputTextarea" style="width:100%" rows="2" maxlength="500"><%=outDataSourceSqlDeal%></textarea>
												</td>
												<td>&nbsp;</td>
											</tr>
										</table>
								...
								<tr>
									<td width="100%" colspan="2">参数设置：</td>
                                </tr>
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding="0" cellspacing="0" border="0" width="100%" id="outSetField">
											<tr>
												<td align="center" style="width:10%">对应sql</td>
												<td align="center" style="width:80%">对应表单字段</td>
												<td align="center" style="width:10%">操作</td>
											</tr>
											<%
											for(int j=0;j<outDataSourceFieldsDealObj.length;j++){
												String[] param_set =outDataSourceFieldsDealObj[j].toString().split("\\|");
											%>
											<tr>
												<td align="center"><input name="outDataSourceFieldsNo" style="width:90%" class="inputText"  value="<%=param_set[0]%>"></td>
												<td align="center">
													<select name="outDataSourceFields" style="width:90%" class="selectlist">
														<option value="">请选择...</option>
													</select>
												</td>
												<td align="center">
													<img width="16" height="16" style="cursor:pointer" border="0"  title="添加" src="images/addarrow.gif" alt="添加赋值字段" onclick="addOutSetField(this);"><img width="15" height="15" style="cursor:pointer" border="0" title="删除" src="images/delarrow.gif" alt="删除赋值字段" onclick="delOutSetField(this);">
												</td>
											</tr>
											<%}%>
										</table>
								...
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding=0 cellspacing=0 border=0 width=100%>
											<tr>
												<td style="width:99%">
													<textarea name="outDataSourceSql" class="inputTextarea" style="width:100%" rows="2" maxlength="500"></textarea>
												</td>
												<td>&nbsp;</td>
											</tr>
										</table>
								...
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding="0" cellspacing="0" border="0" width="100%" id="outSetField">
											...
											<tr>
												<td align="center"><input name="outDataSourceFieldsNo" style="width:90%" class="inputText"  value=""></td>
												<td align="center">
													<select name="outDataSourceFields" style="width:90%" class="selectlist">
														<option value="">请选择...</option>
													</select>
								...
								<!-----------退回执行------------->
								<tr>
									<td width="100%" colspan="2" style="font-weight:bold;"><s:text name="workflow.newactivityreturnmethod"/>：</td>
                                </tr>
								<%
								if(outDataSourceListBack !=null && outDataSourceListBack.size() >0){
									for(int i=0;i<outDataSourceListBack.size();i++){
										Object[] objBack =(Object[])outDataSourceListBack.get(i);
										String outDataSourceSqlBack =objBack[1]==null?"":objBack[1].toString();
										String outDataSourceFieldsBack =objBack[2]==null?"":objBack[2].toString();
										String[] outDataSourceFieldsBackObj =outDataSourceFieldsBack.split(",");
								%>
								<tr>
									<td width="100%" colspan="2">关联条件：<span class="MustFillColor">注：在输入框中输入SQL语句（如：insert into demo_user (user_id,user_name) values (hibernate_sequence.nextval, ?)|update demo_user set user_address =? where user_name =?），可写多条SQL语句，用“|”分隔。</span></td>
								</tr>
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding=0 cellspacing=0 border=0 width=100%>
											<tr>
												<td style="width:99%">
													<textarea name="outDataSourceSqlBack" class="inputTextarea" style="width:100%" rows="2" maxlength="500"><%=outDataSourceSqlBack%></textarea>
												</td>
												<td>&nbsp;</td>
											</tr>
										</table>
								...
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding="0" cellspacing="0" border="0" width="100%" id="outSetFieldBack">
											<tr>
												<td align="center" style="width:10%">对应sql</td>
												<td align="center" style="width:80%">对应表单字段</td>
												<td align="center" style="width:10%">操作</td>
											...
											<%
											for(int j=0;j<outDataSourceFieldsBackObj.length;j++){
												String[] paramback_set =outDataSourceFieldsBackObj[j].toString().split("\\|");
											%>
											<tr>
												<td align="center"><input name="outDataSourceFieldsBackNo" style="width:90%" class="inputText"  value="<%=paramback_set[0]%>"></td>
												<td align="center">
													<select name="outDataSourceFieldsBack" style="width:90%" class="selectlist">
														<option value="">请选择...</option>
													</select>
												</td>
												<td align="center">
													<img width="16" height="16" style="cursor:pointer" border="0"  title="添加" src="images/addarrow.gif" alt="添加赋值字段" onclick="addOutSetFieldBack(this);"><img width="15" height="15" style="cursor:pointer" border="0" title="删除" src="images/delarrow.gif" alt="删除赋值字段" onclick="delOutSetFieldBack(this);">
												</td>
											</tr>
											<%}%>
										</table>
								...
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding=0 cellspacing=0 border=0 width=100%>
											<tr>
												<td style="width:99%">
													<textarea name="outDataSourceSqlBack" class="inputTextarea" style="width:100%" rows="2" maxlength="500"></textarea>
												</td>
												<td>&nbsp;</td>
											</tr>
										</table>
								...
								<tr>
									<td width="100%" colspan="2">
										<table cellpadding="0" cellspacing="0" border="0" width="100%" id="outSetFieldBack">
											...
											<tr>
												<td align="center"><input name="outDataSourceFieldsBackNo" style="width:90%" class="inputText"  value=""></td>
												<td align="center">
													<select name="outDataSourceFieldsBack" style="width:90%" class="selectlist">
														<option value="">请选择...</option>
													</select>
...
function checkDS(){
    var data = "ds="+$('select[name=ds]').val();
	$.ajax({
		type: 'POST',
		url: '<%=rootPath%>/platform/custom/custom_database/dropdownselect/testDataSource.jsp?time='+(new Date().getTime()),
		cache: false,
		async: false,
		data: data,
		success: function(response) {
            var ret = response.replace(/\r|\n/igm,'');
			if(ret!='true'){
                alert("数据源连接失败，请检查数据源配置文件！");
            }else{
                alert("数据源连接成功！");
            }
		}
...
function addOutSetField(obj){
	var secondRow = $('#outSetField').find('tr:eq(1)');
	var _html = $(secondRow).html();
	//alert(_html);
	var tr = $('<tr>'+_html+'</tr>');
	$(tr).find('select[name=outDataSourceFieldsNo]').val('');
	$(tr).find('select[name=outDataSourceFields]').val('');
    var parentTr = $(obj).parent().parent();
    $(parentTr).after(tr);
}

var MSG_CONFIRM = "确定要删除吗？";
function delOutSetField(obj){
	var parentTr = $(obj).parent().parent();
    var tr = $('#outSetField').find('tr');
    if($(tr).length>2){
        /*if(confirm(MSG_CONFIRM)){
            $(parentTr).remove();
        }*/
        whir_confirm(MSG_CONFIRM, function(){
            $(parentTr).remove();
        });
    }else{
    }
...
function addOutSetFieldBack(obj){
	var secondRow = $('#outSetFieldBack').find('tr:eq(1)');
	var _html = $(secondRow).html();
	//alert(_html);
	var tr = $('<tr>'+_html+'</tr>');
	$(tr).find('select[name=outDataSourceFieldsBackNo]').val('');
	$(tr).find('select[name=outDataSourceFieldsBack]').val('');
    var parentTr = $(obj).parent().parent();
    $(parentTr).after(tr);
}

function delOutSetFieldBack(obj){
	var parentTr = $(obj).parent().parent();
    var tr = $('#outSetFieldBack').find('tr');
    if($(tr).length>2){
        /*if(confirm(MSG_CONFIRM)){
            $(parentTr).remove();
        }*/
