...
<SCRIPT type="text/javascript">    
	...
	%>
		  ...
		  <%}else{%>
			<%if(add_dh){%>,<%}else{add_dh=true;}%>{ id:<%=Id%>, pId:<%=ParentId%>, name:"<%=ModName%>", target:'mainFrame', open:true, url:"<%=rootPath%>/custinfo!custRightList.action?CENTERID=<%=Id%>&TableId=<%=TableId%>&ModClassId=<%=ModClassId%>&rightcode=<%=rightcode%>&rightscope=<%=rightscope%>&rightcodemanage=<%=rightcodemanage%>",iconSkin:"fa fa"}
		  <%}%>
