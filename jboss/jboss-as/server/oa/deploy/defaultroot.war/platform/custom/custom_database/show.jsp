    ...
    <tr class="<%=listClass%>">
        ...
        %> 
            <select name="fieldShow" onchange="update(<%=i%>)" style="width:180px;" class="selectlist">
				...
				<option value="216" <%="216".equals(showId)?"selected":""%>>多选组织和人员</option>
				</optgroup>
				<!--<optgroup label="编号类型">
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td align=center>
                    ...
                    <font id="remark116" name="remark116" style="display:none"><label class="MustFillColor">用于存储从Word编辑器中获取的数据。（适用于字符型字段和文本型字段，显示附件按钮只支持ezFLOW流程引擎）</label></font>
                    <font id="remark117" name="remark117" style="display:none"><label class="MustFillColor">用于存储从Excel编辑器中获取的数据。（只适用于字符型字段，显示附件按钮只支持ezFLOW流程引擎）</label></font>
                    <font id="remark118" name="remark118" style="display:none"><label class="MustFillColor">用于存储从WPS编辑器中获取的数据。（只适用于字符型字段，显示附件按钮只支持ezFLOW流程引擎）</label></font>
					...
					<font id="remark216" name="remark216" style="display:none"><label class="MustFillColor">用于存储用户选择的多个组织和人员。</label></font>
                    ...
                    <font id="remark808" name="remark808" style="display:none"><label class="MustFillColor">用于两个时间段内的日期时间计算，“只计算工作日”的选项只支持ezFLOW流程引擎。</label></font>
...
function saveFormData(flag, obj){
	...
	//控制外部接口数据选择参数
	 var outterInterface = $("#outterInterface");
	 if(outterInterface){
		var outterParent = $(outterInterface).parent();
		var outterDispaly = $(outterParent).css('display');
		if(outterDispaly!="none"){
			if($(outterInterface).val()==""){
				 whir_alert("外部接口数据设置不能为空！", null);
				 return false;
			}
		}
	 }
