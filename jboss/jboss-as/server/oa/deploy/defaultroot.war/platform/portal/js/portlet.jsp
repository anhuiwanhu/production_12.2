...
//20161011 -by jqq 增加了portlet外层与内容结构样式设计
var PORTLET_INNERTEMP_SETTING_URL = PORTAL_PATH+"/platform/portal/layout/common/portletInnerTemp.jsp";
<%}%>
...
var Portlet = {
    ...
    setPortletDataTitle: function(portletSettingId, jsonData){
        ...
        //更新主标题
        if(dataLen > 0){
            hObj.append('<h3 id="h_portletSettingId_'+portletSettingId+'" '+(reLiWidth?' style="right:60px;"':'')+'><a href="javascript:void(0);" id="mt_portletSettingId_'+portletSettingId+'" style="color:'+_color+'">'+hObj.attr('data-title')+'</a></h3>');
        }
	...
	addPortlet: function(opts){
        ...
        //20160628 -by jqq 新增加内边距/圆角等样式
		var hasPadding = opts.hasPadding?opts.hasPadding:"";
		var hasBorderRadius = opts.hasBorderRadius?opts.hasBorderRadius:"";
        ...
        if(flag==1){
            <%if(isSameSession){%>
        module += '        <i id="edit_'+pid+'" title="内容设置" class="fa fa-pencil fa-color"></i>';
				if(type == 'information'){
					//module += '        <i id="outer_'+pid+'" title="外层样式设置" class="fa fa-skin fa-color"></i>';
					module += '        <i id="inner_'+pid+'" title="样式设置" class="fa fa-cog fa-color"></i>';
				}
        ...
        if(flag==0){
			...
			//20170531 -by jqq 信息门户登录前增加more链接
			if(type == 'information'){
					module += '<i id="more_'+pid+'" title="更多" class="fa fa-more fa-color" style=""></i>';
			}
        }
        module += '    </div>';
        //-- toolbar end--
		...
		//20161011 -by jqq 新增外层与内层结构样式设置
		$('#inner_'+pid).bind('click', function(){
			portletInnerSetting(p, layoutId, portletId, portletSettingId, type, title);
		});
		...
		//20160628 -by jqq 新增圆角/内边距等样式配置
		if(opts.hasPadding=='1'){
			$('#'+pid).addClass('wh-portal-nopadding');
		}else{
			$('#'+pid).removeClass('wh-portal-nopadding');
		}
		if(opts.hasBorderRadius=='1'){
			$('#'+pid).addClass('wh-portal-radius');
		}else{
			$('#'+pid).removeClass('wh-portal-radius');
		}
	...
	getCommonOpts: function(ps, opts){
		...
		//20160628 -by jqq 新增圆角/内边距等样式配置
		var hasPadding = $(ps).find('hasPadding:first').text();
		var hasBorderRadius = $(ps).find('hasBorderRadius:first').text();
		...
		opts = $.extend(opts, {title:title, titleColor:titleColor, contentBgColor:contentBgColor, iconCls:iconCls, iconFont:iconFont, height:height, hasTitle:hasTitle, hasToolbar:hasToolbar, hasBorder:hasBorder, hasPadding:hasPadding, hasBorderRadius:hasBorderRadius});
...
function portletSetting(target, layoutId, portletId, portletSettingId, type, title){
	var data = '&layoutId='+layoutId+'&portletId='+portletId+'&portletSettingId='+portletSettingId+'&type='+type+'&outter=<%="outter".equals(request.getParameter("type"))?"1":"0"%>&portalType=<%=com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("type"))%>';
...
//20161011 -by jqq portlet外层与内层结构样式设置
function portletInnerSetting(target, layoutId, portletId, portletSettingId, type, title){
	var data = '&layoutId='+layoutId+'&portletId='+portletId+'&portletSettingId='+portletSettingId+'&type='+type+'&outter=<%="outter".equals(request.getParameter("type"))?"1":"0"%>&portalType=<%=com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("type"))%>';
	var api1 = $.dialog({id:portletSettingId,title:title+'内层结构样式设置',width:730,height:150,lock:true,ok:function(){return saveInnerSetting();},okVal:'保存',cancel:true,cancelVal:'取消',max:false,min:false,resize:false}); 
	api = api1;
	$.ajax({ 
		url:PORTLET_INNERTEMP_SETTING_URL+'?time='+(new Date().getTime())+data, 
		success:function(data){ 
			api.content(data); 
		}, 
		cache:false 
